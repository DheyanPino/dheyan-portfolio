...
const ProcessInput: React.FC<{
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-[8px_8px_0px_rgba(0,0,0,0.8)] border-4 border-black w-full">
      <h3 className="text-2xl font-bold text-gray-800 mb-6 border-b-2 border-dashed pb-2">
        Add New Process
      </h3>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            className="w-full p-3 rounded-xl border-2 border-black focus:outline-none focus:ring-2 focus:ring-red-500 font-mono text-lg text-black placeholder-gray-500 shadow-inner"
            placeholder="e.g., 0, 5"
          />
        </div>
        <div>
...
const ProcessTable: React.FC<{
  ...
  if (!processes.length) {
    return (
      <div className="bg-white p-6 rounded-2xl shadow-[8px_8px_0px_rgba(0,0,0,0.8)] border-4 border-black h-80 flex flex-col items-center justify-center w-full">
        <h3 className="text-3xl font-extrabold text-gray-500 text-center">
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-[8px_8px_0px_rgba(0,0,0,0.8)] border-4 border-black w-full overflow-x-auto">
      <h3 className="text-2xl font-bold text-gray-800 mb-6 border-b-2 border-dashed pb-2">
        Process List & Results
      </h3>
      <table className="min-w-full table-auto border-collapse">
        <thead>
          <tr className="text-left bg-gray-100 border-b-2 border-black">
            {headers.map((header) => (
              <th
                key={header}
                className="p-2 text-sm font-extrabold text-gray-700 whitespace-nowrap"
              >
                {header}
              </th>
            ))}
          ...
          {[...processes]
            ...
            .map((p) => {
              ...
              return (
                ...
                >
                  <td className="p-2 font-bold text-indigo-700">{p.name}</td>
                  <td className="p-2 font-mono text-gray-900">{p.at}</td>
                  <td className="p-2 font-mono text-gray-900">{p.bt}</td>
                  <td className="p-2 font-mono text-green-700 font-bold">
                    {calculated?.ct ?? ""}
                  </td>
                  <td className="p-2 font-mono text-red-700 font-bold">
                    {calculated?.tat ?? ""}
                  </td>
                  ...
                  <td className="p-2">
                    <button
                      ...
                      className="p-1 bg-red-100 rounded-full text-red-600 hover:bg-red-500 hover:text-white transition"
...
const MetricsDisplay: React.FC<{ metrics: Metrics }> = ({ metrics }) => (
  <div className="bg-yellow-200 p-6 rounded-2xl shadow-[8px_8px_0px_rgba(0,0,0,0.8)] border-4 border-black grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
    <h3 className="md:col-span-2 text-2xl font-bold text-gray-800 mb-4 flex items-center">
      <Award className="w-6 h-6 mr-2 text-orange-600" /> Average Metrics
    </h3>
    <div className="bg-white p-4 rounded-xl border-2 border-black shadow-inner">
      <p className="text-lg font-semibold text-gray-600">Avg. Waiting Time (WT)</p>
      <p className="text-4xl font-extrabold text-indigo-700 mt-1">
        {metrics.avgWt.toFixed(2)} ms
      </p>
    </div>
    <div className="bg-white p-4 rounded-xl border-2 border-black shadow-inner">
      <p className="text-lg font-semibold text-gray-600">Avg. Turnaround Time (TAT)</p>
      <p className="text-4xl font-extrabold text-indigo-700 mt-1">
...
const GanttChart: React.FC<{ gantt: GanttBlock[]; totalTime: number }> = ({
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-[8px_8px_0px_rgba(0,0,0,0.8)] border-4 border-black mt-8">
      <h3 className="text-2xl font-bold text-gray-800 mb-6 border-b-2 border-dashed pb-2">
      ...
      <div className="overflow-x-auto">
        <div className="relative h-20" style={{ width: `${maxTime * scaleFactor * 2}px`, minWidth: "100%" }}>
          {gantt.map((block, index) => (
            <div
              key={index}
              className={`absolute h-10 top-0 text-white flex items-center justify-center text-sm font-bold border-r-2 border-black ${block.color} transition-all duration-500 ease-out`}
              style={{
          ...
          <div className="absolute top-10 w-full h-10">
            {markers.filter((m) => m <= maxTime).map((time) => (
              <div
                key={time}
                className="absolute h-full border-l border-gray-500 text-xs text-gray-600 pt-1"
...
export default function FCFSSimulator() {
  ...
  return (
    <div className="min-h-screen bg-yellow-100 p-4 md:p-10 font-sans antialiased">
      <header className="text-center mb-10 pt-4">
          <Button variant="secondary" className="mb-4" onClick={() => window.location.href = '/'}>
              &larr; Back to Portfolio
          </Button>
        <h1 className="block text-4xl md:text-5xl font-extrabold text-indigo-900 border-b-8 border-black pb-4">
          <Cpu className="w-10 h-10 inline-block mr-3 text-red-600" /> FCFS
          Scheduler Simulator
        </h1>
